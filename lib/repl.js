...
var addHistory, addMultilineHandler, CS, EmberScript, fs, merge, nodeREPL, path, vm;
fs = require('fs');
path = require('path');
vm = require('vm');
...
module.exports = {
  start: function (opts) {
    ...
    if (null == opts)
      opts = {};
    opts.prompt || (opts.prompt = 'emberscript> ');
    ...
    opts['eval'] || (opts['eval'] = function (input, context, filename, cb) {
      ...
      if (/^\s*$/.test(input))
        return cb(null);
      try {
        inputAst = EmberScript.parse(input, {
          filename: filename,
          raw: true
        });
        transformedAst = new CS.AssignOp(new CS.Identifier('_'), inputAst.body);
        jsAst = EmberScript.compile(transformedAst, {
          bare: true,
